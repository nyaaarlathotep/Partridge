...
import cn.nyaaar.partridgemngservice.exception.eh.PiningException;
...
//        galleryDetail.previewSet = parsePreviewSet(document, body);
    ...
    public static GalleryTagGroup[] parseTagGroups(Elements trs) {
        try {
            ...
            for (Element tr : trs) {
                GalleryTagGroup group = parseTagGroup(tr);
                if (null != group) {
                    list.add(group);
                }
            }
    ...
    public static GalleryCommentList parseComments(Document document) {
        try {
            ...
            for (Element c1 : c1s) {
                GalleryComment comment = parseComment(c1);
                if (null != comment) {
                    list.add(comment);
                }
            }
